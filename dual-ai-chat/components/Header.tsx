...
import { AiRoleConfig, FailedStepPayload } from '../types';
import { Cpu, Sparkles, Settings, RefreshCw, Layers } from 'lucide-react';
import { getRoleModelSummary } from '../utils/appUtils';
...
const ModelBadge = ({
  ...
  <div className={`flex items-center px-2 py-1 rounded-lg border bg-opacity-50 ${colorClass} backdrop-blur-sm`} title={`${label}: ${value}`}>
    ...
    <span className="text-xs font-medium truncate max-w-[180px]">{value}</span>
  </div>
);
    ...
    <header className={`h-16 md:h-20 px-4 md:px-6 flex items-center justify-between shrink-0 bg-white/80 backdrop-blur-md border-b border-slate-200 z-30 transition-all duration-300 ${isNotepadFullscreen ? 'opacity-0 pointer-events-none h-0 p-0 overflow-hidden' : ''}`}>
      ...
      <div className="flex items-center space-x-3 md:space-x-4">
        <div className="hidden md:flex items-center space-x-2 bg-slate-100/50 p-1 rounded-xl border border-slate-200/50">
          <ModelBadge icon={Cpu} label="Cognito" value={getRoleModelSummary(cognitoConfig)} colorClass="bg-indigo-50 border-indigo-100 text-indigo-700" />
          <ModelBadge icon={Sparkles} label="Muse" value={getRoleModelSummary(museConfig)} colorClass="bg-fuchsia-50 border-fuchsia-100 text-fuchsia-700" />
        </div>
